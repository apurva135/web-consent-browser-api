  ...
  chrome.tabs.query(
    ...
    (tabs) => {
      ...
      GDPRConfig.getDebugValues().then((settings) => {
        ...
        document
          .querySelector("#unhandled_cancel")
        ...
        document
          .querySelector("#unhandled_send")
          .addEventListener("click", () => {
            console.log("Clicked");
            fetch(
              "https://gdprconsent.projects.cavi.au.dk/report.php?url=" +
                encodeURIComponent(url)
            );
            document.querySelector(".unhandledtab").style.display = "none";
            document.querySelector(".unhandledtab_complete").style.display =
              "block";
            setTimeout(() => {
              window.close();
            }, 1250);
