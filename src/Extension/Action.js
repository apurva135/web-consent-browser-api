  ...
  async execute(param) {
    ...
    if (result.target != null) {
      ...
      if (ConsentEngine.debugValues.debugClicks) {
        // chrome.storage.local.set({ sample: result.target.innerText });
        console.log(
          "Clicking: [openInTab: " + this.config.openInTab + "]",
          result.target
        );
        const element = result.target.tagName;
        console.log("element clicked", result.target.tagName);
        if (element.toLowerCase() === "input") {
          console.log("Inputtttttttttttttttttt");
          const labelVar = result.target.getAttribute("id");
          const labelName = document
            .querySelector(`label[for="${labelVar}"] span`)
          ...
          //   if (!entries[web][labelName]) {
          //     entries[web][labelName] = true;
          //     await GDPRConfig.setReceipt(entries);
          //     console.log("Updated receipt with new labelName:", entries);
          //   } else {
          //     console.log("LabelName already exists in the receipt.");
          //   }
          // });
        }
      }
      ...
      if (this.config.openInTab) {
        ...
        console.log("Diff behaviour", result.target);
        result.target.dispatchEvent(
          new MouseEvent("click", { ctrlKey: true, shiftKey: true })
        );
      } else {
        console.log("Diff behaviour 2", result.target);
        result.target.click();
      }
...
class MultiClickAction extends Action {
  ...
  async execute(param) {
    ...
    if (ConsentEngine.debugValues.debugClicks) {
      console.log("MultiClicking:", results[1]);
    }
        ...
        if (negated) {
          if (result.target != null) {
            if (numRetries > 0) {
              ...
              if (ConsentEngine.debugValues.debugLog) {
                console.timeEnd(
                  "Waiting [" + negated + "]:" + self.config.target.selector
                );
              }
              resolve();
            }
...
class HideAction extends Action {
  ...
  async execute(param) {
    ...
    if (result.target != null) {
      ...
      if (
        ConsentEngine.generalSettings.hideInsteadOfPIP ||
        this.config.forceHide === true
      ) {
        ...
        function setStyles() {
          ...
          result.target.style.setProperty(
            "transform",
            "translateY(-4rem) scale(" + scale + ")",
            "important"
          );
          result.target.style.setProperty(
            "transform-origin",
